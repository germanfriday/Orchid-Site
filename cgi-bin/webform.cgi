...
use webform_referers qw(@referers);
...
sub return_html {
    ...
    # Otherwise, begin printing the response page.                           #
        ...
        # Otherwise, use the order the fields were sent, or alphabetic.      #
            ...
            foreach $field (sort keys %Config) {
                local $fname = &clean_html($field);
                print "<b>$fname:</b> $Config{$field}<p>\n";
            }
...
sub db_open {  
  # passed paramters:
  #  $dbhost		host name of database installation
  #  $dbname		name of data base
  #  $dbuser		user name to connect as
  #  $dbpasswd		password for $dbuser
  # returned:
  #   1 = connection successful
  my ($dbhost,$dbname,$dbuser,$dbpasswd) = @_;
  
  my $dbh = DBI->connect("DBI:mysql:database=$dbname:host=$dbhost",$dbuser,$dbpasswd,{PrintError => 0});
  my $string = defined $dbh ? "": DBI->errstr;
  return ($dbh,$string);
}
...
sub db_logerror {
  ...
  if ((defined $dbh) && $errstr) {
    $rows_affected = $dbh->do(q{
      INSERT INTO errorlog (errortext) VALUES ($errstr);
    }, undef);
  }
}

sub save_form {
  my ($dbh, $sth, $errstr, $rows_affected, $seq);
  ($dbh, $errstr) = db_open('localhost','mci_webdb','mci_sqladm','pa55w0rd');
  if ($errstr) {
    $Config{ERROR} += $errstr."\n";
  }
  my $form_id = $Config{FORM_ID};
  if (defined $dbh) {
    # default any empty fields
    my $sth = $dbh->prepare(q{
      SELECT * FROM webforms WHERE form_id = ?;
    });
    if ($sth->execute($form_id)) {
      if (my $row = $sth->fetchrow_hashref()) {
        my $form_fields = $$row{form_fields};
        $form_fields =~ s/\r\n/ /g;
        $form_fields =~ s/  / /g;
        my @formfields = split(" ", $form_fields);
        foreach my $f (@formfields) {
          unless(defined $Form{$f}) {
            $Form{$f} = '';
          }
        }
      }
    ...
    $seq = 0;
    $sth = $dbh->prepare(q{
      SELECT MAX(form_seq) AS max_form_seq FROM webforms_seq WHERE form_id = ?;
    });
    if ($sth->execute($form_id)) {
      if (my $row = $sth->fetchrow_hashref()) {
        $seq = $$row{max_form_seq};
      }
    } 
    $sth->finish();        
    $seq++;
    $dbh->do(q{
        INSERT INTO webforms_seq (form_id, form_seq) VALUES (?,?);
      }, undef, $form_id, $seq) or db_logerror($dbh, $dbh->errstr);
#$Config{DEBUG} = $Config{FORM_ID}."<br>"; 
    # save form data
    foreach my $field (keys %Form) { 
      $dbh->do(q{
        INSERT INTO webforms_data (form_id, form_seq, field_name, field_value) VALUES (?,?,?,?);
        }, undef, $form_id, $seq, $field, $Form{$field}) or db_logerror($dbh, $dbh->errstr);
#$Config{DEBUG} .= join('|', ($Config{FORM_ID}, $seq, $field, $Form{$field}))."<br>";
    }
    $dbh->disconnect;
  }
}
